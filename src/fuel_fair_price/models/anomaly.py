...
def robust_zscore(values: pd.Series) -> pd.Series:
    """Median/MAD robust z-score; returns 0 when a peer group has zero MAD."""
...
def score_stations(
    ...
    """
    Compute raw fair-price spread and a peer-relative robust anomaly score.

    The z-score is computed among comparable stations rather than nationally,
    reducing false positives caused by regional or motorway effects.
    """
    out = stations.copy()
    ...
    if valid_peer_columns:
        out["anomaly_z"] = (
            out.groupby(valid_peer_columns, dropna=False)["spread_eur_l"]
